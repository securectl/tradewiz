...
def api_oversold_export():
    """Export oversold candidates as CSV / TXT / PDF.

    Query params:
        format=csv|txt|pdf  (default csv)
        start=YYYY-MM-DD    (default: 30 days ago)
        end=YYYY-MM-DD      (default: today)
        dedupe=1|0          (default 1; collapse repeat sightings per ticker)
    """
    fmt = (request.args.get("format", "csv") or "csv").lower()
    if fmt not in ("csv", "txt", "pdf"):
        return jsonify({"error": "format must be csv, txt, or pdf"}), 400
