...
function applyUserRole() {
    ...
    const isTrader = currentUser.roles && (currentUser.roles.includes('trader') || currentUser.roles.includes('admin'));
    ...
    try {
        ...
        if (data.ok) {
            ...
            if (broker === 'webull') {
                ...
                document.getElementById('sbot-cfg-wb-account-id').value = '';
            } else {
...
async function loadAdminUsers() {
    try {
        ...
        if (!tbody) return;
        if (!users.length) {
            tbody.innerHTML = '<tr><td colspan="5" class="bot-empty">No users</td></tr>';
            return;
        }
        tbody.innerHTML = users.map(u => `
            <tr>
                ...
                <td>
                    <select onchange="adminChangeRole(${u.id}, this.value)" style="font-size:11px; background:var(--bg-tertiary); color:var(--text-primary); border:1px solid var(--border-color); border-radius:4px; padding:2px 4px;">
    ...
    try {
        ...
        list.innerHTML = data.invites.map(inv => `
            <div class="admin-invite-item">
                ...
                <button class="btn-revoke" onclick="adminRevokeInvite('${inv.email}')">Revoke</button>
            </div>
        `).join('');
    } catch (e) {
